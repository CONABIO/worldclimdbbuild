...
# se definen variables para la divsión de bins para esta y la siguietne sección
...
try:
    ...
    cur.execute(bioclim_vars_info_sql.format(source=source, description=description + " Num. bins:" + nbins))
    logger.info('bioclim_vars_info_sql')

    cur.close()
    conn.close()

except Exception as e:
    logger.error('No se pudo crear la tabla de fuentes climaticas y/o raster bins: {0}'.format(str(e)))
    sys.exit()
...
try:
    ...
    for source in sources:
        ...
        with open(tags, 'r') as f:
            ...
            for indice, line in enumerate(reader):
                ...
                if contador-1 > indice:
                	ifelse_query = ifelse_query + " WHEN (DN / 100.0) BETWEEN " + minval + " AND " + maxval + " THEN '" + layer + "' "
                else:
                	ifelse_query = ifelse_query + " ELSE '" + layer + "' "

            ifelse_query = ifelse_query + " END AS categoria, " + icat + " as icat "

        # ifelse_query = " CASE WHEN DN >= -21.85932502746582 AND DN <= -27.218530654907227 THEN 'bio001-1' ELSE 'bio001-2' END AS categoria "

        logger.info('Se insertan etiquetas (rangos)')
        logger.info('ifelse_query: {}'.format(ifelse_query))
        ...
        query_shape = 'SELECT *, ' + nbins + ' as nbins, (DN / 100.0) AS value, ' + ifelse_query + ' FROM ' + final_shape_name + ' WHERE DN < 1000000 and DN > -1000000'
...
#     logger.info('Iniciando Conexion con DB')
...
#             if not line[2] in sources:
#                 sources.append(line[2])
#                 sourceid += 1

#             # args = ['python3', 'get_region_vars.py', '{0}{1}'.format(data_folder, line[0])]
#             # p = subprocess.Popen(args, stdout=subprocess.PIPE)
#             # reg = p.communicate()[0].decode("utf-8")

#             # regions.append(reg)
#             # logger.info('region añadida al array: {0}'.format(reg))
            
#             # se comenta linea que hace update en la tabla raster_bins
#             cur.execute(update_label_var_sql.format(label=line[1], type=sourceid, layer='bio{0}'.format(get_three_digits_number_var(layerid))))

#             if not line[3] in descriptions:
#                 descriptions.append(line[3])

#     # for i in range(sourceid):
#         # cur.execute(bioclim_vars_info_sql.format(source=sources[i], description=descriptions[i], regions=regions[i]))
#     cur.execute(bioclim_vars_info_sql.format(source=sources[-1], description=descriptions[-1]))
